...
- **Product name** — Command center branding is **Scout** (`package.json` `scout`, `public/Scout_Logo.png` with legacy `Projectr_Logo.png` fallback in PDF loader); workspace session keys remain **`projectr-*`** per `lib/local-workspace-reset.ts` and `lib/agent-chat-storage-key.ts`.
