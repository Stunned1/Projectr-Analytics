...
- Left sidebar now collapsible — collapses to 48px icon strip, expands to **240px**; top zone = logo, search, Map/Client CSV nav; scrollable middle = shortlist only; **Active market** card (or collapsed map icon) sits in the **footer** and toggles the right data panel; intelligence terminal opens only from the map-bottom bar (no sidebar AI button).
...
- **Themed scrollbars** — dark mode uses global native scrollbar styling (`html.dark *` in `globals.css`: thin rail, cool graphite track, muted thumb, primary tint on hover); `.scrollbar-none` keeps the bottom stats pill’s horizontal strip chromeless; shadcn **ScrollArea** thumb uses muted/primary hover for custom rails.
