...
| Surface | Disposition |
|---|---|
| `/api/agent` prompt + routing | Narrowed to a one-pass EDA contract with deterministic fallback summaries, a shared request-lane classifier, context-aware upload-vs-market subject selection, and a bounded Austin-vs-Houston / Austin-vs-Dallas Overture core-retail comparison lane; explicit map controls remain available as a separate direct-action lane. |
...
- Phase-2 demo branch merges now carry forward the local agent grounding validator and Google auth helpers so the Austin-based worktree can boot the bounded agent route and tests without missing-module failures.
