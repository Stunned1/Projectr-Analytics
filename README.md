...
- **Case brief PDF** — after ranked analysis site cards, **Download case brief (PDF)** calls `/api/agent/case-brief/pdf` (Gemini + `@react-pdf`); browser downloads `Projectr-Case-Brief-{market}.pdf` with the same structured sections as the market PDF (dark header band, orange accent, signal tiles, ranked sites, methodology).
- **City search** — non-ZIP queries accept full state names (`Newark, New Jersey`) or abbreviations (`Newark, NJ`); search placeholder notes “City & State”.
