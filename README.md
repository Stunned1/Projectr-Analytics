...
- `POST /api/agent/case-brief/pdf` - same payload as case-brief; runs Gemini then renders `@react-pdf/renderer` `CaseBriefPdfDocument` and returns `application/pdf` with `Content-Disposition: attachment` (shared prompt/builder in `lib/report/case-brief-shared.ts`); PDF header uses `public/scout.png` (or legacy `Scout_Logo.png` / `Projectr_Logo.png` via loader fallback) when present, includes market snapshot from `mapContext`, composite score chart and site metrics table from raw sites, paginated per-site narratives with FAR bars, and expanded brief sections (findings, thesis, risks/mitigations, next steps, assumptions).
...
- **Product name** — Command center branding is **Scout** (`package.json` `scout`, sidebar and PDF loader prefer `public/scout.png`, then `Scout_Logo.png` / `Projectr_Logo.png`); workspace session keys remain **`projectr-*`** per `lib/local-workspace-reset.ts` and `lib/agent-chat-storage-key.ts`.
- **`scout.png`** — `CommandCenterSidebar` header image and `loadScoutLogoDataUri()` try `public/scout.png` before legacy logo filenames for PDF headers.
- **Sidebar header** — white **Scout** wordmark beside the logo image; **Testing** strip (clear-local button) removed from the nav — use **`/clear:workspace`** on the map intelligence terminal to wipe session keys and reload.
