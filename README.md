...
- Left sidebar now collapsible — collapses to 48px icon strip, expands to **240px**; top zone = logo, search, Map/Client CSV nav; scrollable middle = shortlist + compact **Active market** (name + cycle stage/position); bottom = **AI** only; docked agent chat offsets by `SIDEBAR_EXPANDED_PX` (15rem).
